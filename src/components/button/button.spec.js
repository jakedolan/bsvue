...
describe('button', () => {
  ...
  /**
   * This is a bad test as there appears to not be a way to check if stopImmediatePropagation has been called nor does there seem to be a way to stop the event.
   * The native on.click will always be called. I don't know how to test or prevent it won't be picked up by further listeners.
   */
  it('should not emit click event when clicked and aria-disabled', async () => {
    let called = 0
    const wrapper = mount(BButton, {
      propsData: {
        ariaDisabled: true
